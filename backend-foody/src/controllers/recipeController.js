...
const  add = async (req,res,state) =>{  
    try{
        ...
        const [result] = await db.query ("INSERT INTO recipes (image,title,category,preparation_method,preparation_time,difficulty,serving,brief_description,user_id,state) values (?,?,?,?,?,?,?,?,?,?)",
            [recipe.image,recipe.recipe_name,recipe.category,recipe.description,Number(recipe.cookingTime),recipe.difficulty,1,recipe.brief_description,userId,state])
...
const getRecipebYId = async (req,res,id)=>{
    try{
        ...
        ingredients.map(async (item) => {
            ...
            const [[name]] = await db.query("select name from ingredient where id_ingredient = ?", [item.ingredient_id]);
            ingredient["name"] = name.name
            ingredient["quatity"] = item.quantity
...
const getID = async (req, res) => {
    try {
        ...
        console.log("searching for title:", name); // check what's being sent
...
const updateByID = async (req, res, id) => {
    try {
        const { category, recipe_name, difficulty, description, cookingTime, brief_description } = req.body;
        const ingredients = JSON.parse(req.body.ingredients);
        const image = req.body.image || req.body.existingImage;

        await db.query(
            "UPDATE recipes SET title = ?, category = ?, difficulty = ?, preparation_method = ?, preparation_time = ?, brief_description = ?, image = ? WHERE id = ?",
            [recipe_name, category, difficulty, description, cookingTime, brief_description, image, id]
        );
...
module.exports = {add,getRecipe,getRecipePublic,getRecipebYId,getID,deleteByID,updateByID};